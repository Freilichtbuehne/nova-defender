...
local phrases = {
    ...
    -- Anticheat
    ["anticheat_reason"] = "如果玩家使用任何形式的作弊，他将被禁止的原因。",
    ...
    ["anticheat_verify_execution"] = "检查反作弊是否正在运行：\n在玩家收到反作弊后，将请求确认他是否已执行它。然而，这个过程可能会因为几个原因而失败，因此不应设置为'禁止'。",
    ["anticheat_verify_reason"] = "如果反作弊无法加载，玩家将被禁止的原因。",
    ...
    ["anticheat_autoclick_reason"] = "如果玩家使用了自动点击，他将被禁止的原因。",
    ...
    ["security_privileges_group_protection_escalation_reason"] = "如果一个玩家拥有他不应该拥有的受保护用户组，他被踢出或禁止的原因。",
    ...
    ["notify_admin_ban"] = "成功禁止%s。如果他下次加入服务器，将被禁止。",
    ["notify_admin_ban_online"] = "管理员%s禁止了%s。玩家将在几秒钟内被禁止。",
    ["notify_admin_ban_offline"] = "管理员%s禁止了%s。如果他下次加入服务器，将被禁止。",
    ["notify_admin_ban_fail"] = "禁止%s失败：%q",
    ...
    ["notify_banbypass_ban_fail"] = "无法禁止%s的%q：%s",
    ["notify_banbypass_kick_fail"] = "无法踢出%s的%q：%s",

    ["notify_banbypass_bypass_fingerprint_match"] = "%s可能绕过封禁：指纹与被禁止的SteamID %s匹配 | 置信度：%d%%",
    ["notify_banbypass_bypass_fingerprint_match_action"] = "可能绕过封禁：指纹与被禁止的SteamID %s匹配 | 置信度：%d%%",

    ["notify_banbypass_familyshare"] = "可能绕过封禁：被禁止的SteamID %s的家庭共享账户",
    ["notify_banbypass_familyshare_action"] = "可能绕过封禁：被禁止的SteamID %s的家庭共享账户",

    ["notify_banbypass_clientcheck"] = "可能绕过封禁：发现%s的封禁绕过证据 | 证据：%s",
    ["notify_banbypass_clientcheck_action"] = "可能绕过封禁：发现%s的封禁绕过证据 | 证据：%s",

    ["notify_banbypass_ipcheck"] = "可能绕过封禁：与被禁止的玩家%s的IP相同",
    ["notify_banbypass_ipcheck_action"] = "可能绕过封禁：与被禁止的玩家%s的IP相同",
    ...
    -- Server
    ["server_general_suffix"] = "附加到每个踢出、禁止或拒绝消息的文本。例如你的Teamspeak，Discord或其他支持网站。",
    ...
    ["indicator_banned"] = "玩家已经在另一个服务器上被Nova Defender禁止",
    ["indicator_profile_familyshared"] = "玩家有一个家庭共享账户",
    ["indicator_profile_friend_banned"] = "这个玩家的一个Steam朋友已经被Nova Defender禁止",
